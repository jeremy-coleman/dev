...
export class DashboardModel extends ComponentModel {
    @observable sync = new SyncModel();
    @observable private _title : string = "untitled";
    ...
    @observable.ref private _blockSource : ComponentModel;
    ...
    @computed
    get windows() : WindowModel[] {
        return this.findAll(c => c.type === ComponentTypes.window) as WindowModel[];
    }
    ...
    protected _findAllChildren(predicate) : any[] {
        if(this._component) {
    ...
    @action
    close() {
        if(this._component as any) {
            (this._component as any).close();
