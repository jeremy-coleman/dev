...
class ComponentRemoveDialog extends React.Component<IComponentRemoveProps, any> {
    private _onClickCancel = () => this.props.remove.cancel();
    private _onClickSave = () => this.props.remove.save();
    private _onDismissed = () => this.props.remove.cancel();
    ...
    return (
            <Modal showModal={!this.props.remove.active? false : true} handleClose={this._onDismissed}>
            <h1>{`Close ${title}`}</h1>
