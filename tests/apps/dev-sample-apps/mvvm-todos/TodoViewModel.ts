...
import {IViewModel, EventEmitter, input, output} from '@coglite/framework/mvvm'
...
export class TodoViewModel implements IViewModel {
    ...
    save(){
        // actual save logic
        if(window.localStorage){
            window.localStorage.setItem("todos",JSON.stringify(this.todos.map(todo => todo.serialize())))
        }
