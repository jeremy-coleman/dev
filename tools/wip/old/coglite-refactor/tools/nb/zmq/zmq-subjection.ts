...
import { ZMQType } from "./zmq-constants";
...
let zmqSubComplete = () => this.removeAllListeners() && this.close();
    

let next = (e) => this.send(new jmp.Message(e));

export let createSubscriber = (socket) => Subscriber.create(next, err => console.log(err), zmqSubComplete());
