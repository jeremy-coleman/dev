...
const parent = function(path : string) : string {
    if(path && path.length > 0) {
        let lastSepIdx;
        if(path.charAt(path.length - 1) === sep && path.length > 1) {
            lastSepIdx = path.lastIndexOf(sep, path.length - 2);
        } else {
            lastSepIdx = path.lastIndexOf(sep);
        }
        if(lastSepIdx > 0) {
...
const joinPaths = function(...args : any[]) {
