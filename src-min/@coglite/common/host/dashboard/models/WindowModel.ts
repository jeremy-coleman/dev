...
export class WindowModel extends ComponentModel {
    name : string;
    onClose : IConsumerFunc<WindowModel>;
    ...
    get active() {
        ...
        if(manager && manager.type === ComponentTypes.stack) {
            return (manager as StackModel).active === this;
        }
        return false;
    }
    ...
    activate() {
        const manager = this.manager;
        if(manager && manager.type === ComponentTypes.stack) {
            (manager as StackModel).setActive(this); //  as IStack
        }
    }
    ...
    resizeStart(resizeDirection) {
        const mgr = this.manager;
        if(mgr) {
            mgr.resizeStart(this, resizeDirection);
        }
    }
 ...
 // NOTE: for get config, the params are always considered transient / memory only
