...
import * as DESKTOP_SETTINGS from 'electron-settings'
...
class OutputArea extends React.Component<{ store: Store }> {
  ...
  getOutputText(output: any): string | void {
  ...
  handleClick = () => {
    ...
    if (copyOutput) {
      electron.clipboard.write(Anser.ansiToText(copyOutput));

      //electron.notification.addSuccess("Copied to clipboard");
    } 
    else { () => {}
  ...
  render() {
    ...
    if (!kernel) {
      if (DESKTOP_SETTINGS.get("Hydrogen.outputAreaDock")) {
        return <EmptyMessage />;
      } else {
            () => {}
